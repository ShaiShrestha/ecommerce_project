...
const Orders = () => {
  const{backendUrl, token, currency} = useContext(ShopContext); //   Importing products and currency from context
            ...
            <div key={index} className='py-4 border-tt border-b text-gray-700 flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
              <div className='flex items-start gap-6 text-sm'>
                ...
                <div>
                  ...
                  <p className='mt-1'>Date: <span className='text-gray-400' /> {new Date(item.date).toDateString()}</p>
                  <p className='mt-1'>Payment: <span className='text-gray-400' /> {item.paymentMethod}</p>
                </div>
              ...
              <div className='md:w-1/2 flex justify-between'>
                ...
                <button onClick={loadOrderData} className='border px-4 py-2 text0sm font-medium rounded-sm'>TRACK ORDER</button>
              </div>
            </div>
          ))
        }
