...
const Navbar = () => {
    ...
    const{setShowSearch, getCartCount, navigate, token, setToken, setCartItems} = useContext(ShopContext); // Function to get the cart count from context
    ...
    <div className = 'flex items-center justify-between py-5 font-medium'>
     ...
     <ul className = 'hidden sm:flex gap-5 text-sm text-gray-700'>
        <NavLink to='/' className = 'flex flex-col items-center gap-1'>
            <p>HOME</p>
            <hr className='w-2/4 boreder-none h-[1.5px] bg-gray-700 hidden' />
        </NavLink>
      ...
      <div className='flex items-center gap-6'>
        ...
        <div className='group relative'>
            <img onClick={()=> token ? null : navigate('/login')} src={assets.user_icon} className ='w-5 cursor-pointer' alt="" />
            {/* Dropdown menu */}
            {token &&             
        ...
        <div className={`fixed top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'} `}>
            <div className='flex flex-col text-gray-600'>
                <div onClick={()=>setVisible(false)}className='flex items-center gap-4 p-3'>
                    <img className='h-4 rotate-360' src={assets.return_icon} alt =""/>
                    <p>Back</p>
                </div>
